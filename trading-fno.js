...
// ============================================================================
// BANNER: Background F&O Day's P&L + Exposure calculation (no UI side effects)
// Called from initTrading and portfolio auto-refresh to keep banner current
// without requiring the F&O tab to be active.
// ============================================================================

async function trFnoBannerRefresh(forceRefresh) {
    // Fetch contract prices (same as trFnoFetchAndRefresh, minus the render)
    var txns = trFnoGetTxns();
    var symbols = {};
    txns.forEach(function(t) {
        if (t.symbol && t.symbol !== t.short_symbol) {
    ...
    // Compute totals from open positions using contract cache (same logic as trFnoCalcPositions)
    var trades = txns.filter(function(t) { return t.transaction_type === 'BUY' || t.transaction_type === 'SELL'; });
    var totDayPnl = 0, totExposure = 0;

    // Group by underlying → contract group → match openers/closers
    var bySymbol = {};
    trades.forEach(function(t) {
        var key = (t.investor_id || '') + '|' + (t.trader_id || '') + '|' + (t.broker_id || '') + '|' + (t.symbol || '');
        if (!bySymbol[key]) bySymbol[key] = { buys: [], sells: [], symbol: t.symbol };
        ...
        if (t.transaction_type === 'BUY') bySymbol[key].buys.push(entry);
        else bySymbol[key].sells.push(entry);
    });
    ...
    Object.keys(bySymbol).forEach(function(key) {
        ...
        // Default LIFO matching
        var openerOrder = openers.slice().reverse();
        ...
        // Sum open position Day's P&L and exposure
        openerOrder.forEach(function(opener) {
            if (opener.remaining <= 0) return;
            var ppu = opener.qty > 0 ? opener.netAmount / opener.qty : 0;
            var openCost = opener.remaining * ppu;
            totExposure += openCost;
            var contractSym = (opener.txn.symbol || '').replace(/^[A-Z]+:/, '');
